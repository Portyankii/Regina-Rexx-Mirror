...
dnl ---------------------------------------------------------------------------
dnl Check for clock_gettime() function
dnl ---------------------------------------------------------------------------
AC_DEFUN([REG_CHECK_CLOCK_GETTIME],
[
mh_save_libs="$LIBS"

if test "$on_qnx4" = no; then
   AC_SEARCH_LIBS([clock_gettime],[rt],[mh_func_clock_gettime_needs_rt=yes],[mh_func_clock_gettime_needs_rt=no])
   if test "$mh_func_clock_gettime_needs_rt" = yes; then
      MH_EXTRA_LIBS="${MH_EXTRA_LIBS} -lrt"
   fi
fi
...
#      LD_RXLIB_B2="-L. -l${SHLFILE}"
   ...
   *cygwin*)
      ...
      OTHER_INSTALLS=""
      RXSTACK_INSTALL=""
      EXTRATARGET="${LIBPRE}${SHLFILE}${SHLPST}${LIBPST}"
      ;;
   *apple-darwin*)
...
if test [ -f /usr/bin/systemctl -o -f /bin/systemctl ]; then
   ...
   buildreq="systemd"
   AC_MSG_RESULT(systemd "in" $systemdinstallpath)
elif test [ -f /sbin/initctl ]; then
   initinstaller="installupstart"
   AC_MSG_RESULT(upstart "in" $systemdinstallpath)
elif test [ -f /sbin/insserv ]; then
# SuSE sysvinit
   initinstaller="installinsserv"
   AC_MSG_RESULT(insserv "in" $systemdinstallpath)
else
   initinstaller="installsysvinit"
   AC_MSG_RESULT(sysvinit "in" $systemdinstallpath)
fi
